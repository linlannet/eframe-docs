...
onMounted(() => {
  if (location.pathname != '/eframe/docs/') {
    // 以 / 结尾代表指向的是目录，展开目录 手工拼接 md 内容
    if(location.pathname.endsWith("/")){
      defaultUrl.value = location.pathname.replace('/eframe', '')
      let currentNode = getTreeNode()
      // 拼接 md 文档内容
      let content = '# ' + currentNode.label + "\r\n"
      content += currentNode.childNodes.map(item=> `### [${item.label}](/eframe${item.key.replace('.md', '')}) `).join('\r\n')
      emits('menuContent', content)

    }else{
      // 根所路径读取文件和节点
      defaultUrl.value = (location.pathname + ".md").replace('/eframe', '')
      getTreeNode()
      getMdContent()
    } 
  }else{
